...
public class GameServer(string serverType, IPAddress address, int port) : UdpServer(address, port)
{
    ...
    protected override void OnReceived(EndPoint endpoint, byte[] buffer, long offset, long size)
    {
        ...
        Log.Information("Received on {UniqueName} {Id} from {EndPoint}", (nameof(GameServer) + "_" + ServerType), Id, endpoint);
        //Log.Information("Received on {UniqueName} {Id} from {EndPoint}\n{Buffer}", (nameof(GameServer) + "_" + ServerType), Id, endpoint, Convert.ToHexString(buf));
        if (buf.Length == 0)
        {
